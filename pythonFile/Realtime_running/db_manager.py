...
# Khởi tạo một Connection Pool toàn cục cơ bản.
try:
    logger.info(f"Initializing MySQL Connection Pool for host: {DB_CONFIG['host']}...")
...
def initialize_database():
    ...
    for i in range(retries):
        try:
            ...
            if db_pool is None:
                ...
                logger.info("Connection Pool re-initialized during startup.")

            # Lấy 1 kết nối từ pool để kiểm tra và tạo bảng
            conn = db_pool.get_connection()
            logger.info(f"Attempt {i+1}/{retries}: MySQL DB connected successfully.")
            cursor = conn.cursor()
            ...
            if err.errno == errorcode.ER_ACCESS_DENIED_ERROR:
                logger.error("Lỗi xác thực (sai user/pass). Sẽ không thử lại.")
                break
            elif err.errno == errorcode.ER_BAD_DB_ERROR:
                logger.error(f"Database '{DB_CONFIG['database']}' không tồn tại (chưa được tạo bởi service 'db'?). Sẽ không thử lại.")
                break
            else:
                logger.warning(f"Attempt {i+1}/{retries}: DB connection failed (service có thể đang khởi động...): {err}")
    ...
    logger.critical("FATAL: Could not initialize database after all retries. Exiting.")
    return False